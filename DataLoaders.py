...
class DataLoaderSGNS(object):
    """Skipgram(Negative-sampling)のためのデータローダー"""
    def __init__(self, text, device, batch_size, window=3, n_negative=5, weights=None):
        """
        :param text: list of list of int, 単語をIDに変換したデータセット
        :param batch_size: int, ミニバッチのサイズ
        :param window: int, 周辺単語と入力単語の最大距離
        :pram n_negative int, 負例の数
        :pram weights: numpy.ndarray, Negative Samplingで使う確率分布
        """
        self.text = text
        self.batch_size = batch_size
        self.window = window
    ...
    def __next__(self):
        ...
        while len(batch_X) < self.batch_size:
            ...
            # 多項分布による負例のサンプリング
            N = torch.multinomial(self.weights, self.n_negative)
            batch_N.append(N)
        ...
        batch_X = torch.tensor(batch_X, dtype=torch.long, device=self.device)
        batch_Y = torch.tensor(batch_Y, dtype=torch.long, device=self.device)
        batch_N = torch.tensor(batch_N, dtype=torch.long, device=self.device)
